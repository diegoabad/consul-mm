...
function generarPDF(datos) {
  return new Promise((resolve, reject) => {
    try {
      ...
      lineas2Col('Nombre', mostrar(paciente.nombre), 'DNI', vacio(paciente.dni) ? formatDni(paciente.dni) : 'No especificado');
      lineas2Col('Apellido', mostrar(paciente.apellido), 'Fecha de nacimiento', mostrarFecha(paciente.fecha_nacimiento));
      doc.moveDown(1.2);

      // ---------- Información de contacto ----------
      tituloSeccion('Información de contacto');
      lineas2Col('Teléfono', mostrar(paciente.telefono), 'Email', mostrar(paciente.email));
      linea('Dirección', mostrar(paciente.direccion));
      doc.moveDown(1.2);

      // ---------- Cobertura médica ----------
      tituloSeccion('Cobertura médica');
      lineas2Col('Obra social', (paciente.obra_social && String(paciente.obra_social).trim()) ? String(paciente.obra_social).trim().toUpperCase() : 'No especificada', 'Plan', mostrar(paciente.plan));
      linea('N° de afiliado', mostrar(paciente.numero_afiliado, 'No específicado'));
      doc.moveDown(1.2);

      // ---------- Contacto de emergencia ----------
      tituloSeccion('Contacto de emergencia');
      lineas2Col('Nombre', mostrar(paciente.contacto_emergencia_nombre), 'Teléfono', mostrar(paciente.contacto_emergencia_telefono));
      doc.moveDown(1.2);
      ...
      if (profesionalesUnicos.length > 0) {
        profesionalesUnicos.forEach((ev) => {
          const nombreCompleto = `${(ev.profesional_nombre || '').trim()} ${(ev.profesional_apellido || '').trim()}`.trim() || '—';
          const especialidad = (ev.profesional_especialidad && String(ev.profesional_especialidad).trim()) ? String(ev.profesional_especialidad).trim() : 'No especificada';
      ...
      doc.fillColor(COLORS.primary).fontSize(16).font('Helvetica-Bold').text('Evoluciones');
      doc.moveDown(0.8);
